...
from aiida.repository import Repository, FileObject, FileType
...
def test_list_objects(repository, generate_directory):
    ...
    assert all([isinstance(obj, FileObject) for obj in objects])
...
def test_walk(repository, generate_directory):
    ...
    generator = repository.walk()
    assert next(generator) == (pathlib.Path('empty'), [], [])
    assert next(generator) == (pathlib.Path('relative/sub'), [], ['file_c'])
    assert next(generator) == (pathlib.Path('relative'), ['sub'], ['file_b'])
    assert next(generator) == (pathlib.Path('.'), ['empty', 'relative'], ['file_a'])
