...
from .common import FileObject, FileType
...
class Repository:
    ...
    def from_serialized(cls, backend: AbstractRepositoryBackend, serialized: typing.Dict) -> 'Repository':
        ...
        if serialized:
            for name, obj in serialized['o'].items():
                instance.get_directory().objects[name] = FileObject.from_serialized(obj, name)
    ...
    def _pre_process_path(path: FilePath = None) -> typing.Union[pathlib.Path, None]:
        ...
        if path.is_absolute():
            raise TypeError('path `{}` is not a relative path.'.format(path))
    ...
    def _insert_file(self, path: pathlib.Path, key: str):
        ...
        directory.objects[path.name] = FileObject(path.name, FileType.FILE, key)

    def create_directory(self, path: FilePath) -> FileObject:
        ...
        for part in path.parts:
            if part not in directory.objects:
                directory.objects[part] = FileObject(part)
    ...
    def get_object(self, path: FilePath = None) -> FileObject:
        """Return the object at the given path.

        :param path: the relative path where to store the object in the repository.
        :return: the `FileObject` representing the object located at the given relative path.
        :raises TypeError: if the path is not a string or ``Path``, or is an absolute path.
        ...
        for part in path.parts:
            if part not in file_object.objects:
                raise FileNotFoundError('object with path `{}` does not exist.'.format(path))

            file_object = file_object.objects[part]

        return file_object

    def get_directory(self, path: FilePath = None) -> FileObject:
        ...
        if file_object.file_type != FileType.DIRECTORY:
            raise NotADirectoryError('object with path `{}` is not a directory.'.format(path))

        return file_object

    def get_file(self, path: FilePath) -> FileObject:
        ...
        if file_object.file_type != FileType.FILE:
            raise IsADirectoryError('object with path `{}` is not a file.'.format(path))

        return file_object

    def list_objects(self, path: FilePath = None) -> typing.List[FileObject]:
        """Return a list of the objects contained in this repository sorted by name, optionally in given sub directory.

        :param path: the relative path of the directory.
        :return: a list of `FileObject` named tuples representing the objects present in directory with the given key.
        :raises TypeError: if the path is not a string or ``Path``, or is an absolute path.
        :raises FileNotFoundError: if no object exists for the given path.
        :raises NotADirectoryError: if the object at the given path is not a directory.
        """
    ...
    def put_object_from_tree(self, filepath: FilePath, path: FilePath = None):
        ...
        if not isinstance(filepath, pathlib.Path):
            raise TypeError('filepath `{}` is not of type `str` nor `pathlib.Path`.'.format(filepath))

        if not filepath.is_absolute():
            raise TypeError('filepath `{}` is not an absolute path.'.format(filepath))
    ...
    def delete_object(self, path: FilePath, hard_delete: bool = False):
        ...
        if file_object.file_type == FileType.DIRECTORY:
            raise IsADirectoryError('object with path `{}` is a directory.'.format(path))
